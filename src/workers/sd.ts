...
export const sd = (returnsPeriod: 'daily' | 'monthly', phMap: TDatePriceMap, startDate: string, endDate: string) => {
  const values: number[] = [];
  ...
  return result * 100 * (returnsPeriod === 'daily' ? ROOT_DAILY : ROOT_MONTHLY);
};
